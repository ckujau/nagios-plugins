...
if   [ $FANSPEED -ge $FANSPEED_CRIT ]; then
	printf "CRITICAL: fanspeed=$FANSPEED "
	ERROR_FANSPEED=2

elif [ $FANSPEED -ge $FANSPEED_WARN ]; then
	printf "WARNING: fanspeed=$FANSPEED "
	ERROR_FANSPEED=1

elif [ $FANSPEED -lt $FANSPEED_WARN ]; then
	printf "OK: fanspeed=$FANSPEED "
	ERROR_FANSPEED=0

else
	# We should never get here
	printf "UNKNOWN: fanspeed=$FANSPEED "
	ERROR_FANSPEED=3
fi

### SENSOR1
if   [ $SENSOR1 -ge $SENSOR1_CRIT ]; then
	printf "CRITICAL: sensor1=$SENSOR1 "
	ERROR_SENSOR1=2

elif [ $SENSOR1 -ge $SENSOR1_WARN ]; then
	printf "WARNING: sensor1=$SENSOR1 "
	ERROR_SENSOR1=1

elif [ $SENSOR1 -lt $SENSOR1_WARN ]; then
	printf "OK: sensor1=$SENSOR1 "
	ERROR_SENSOR1=0

else
	# We should never get here
	printf "UNKNOWN: sensor1=$SENSOR1 "
	ERROR_SENSOR1=3
fi

### SENSOR2
if   [ $SENSOR2 -ge $SENSOR2_CRIT ]; then
	printf "CRITICAL: sensor2=$SENSOR2 "
	ERROR_SENSOR2=2

elif [ $SENSOR2 -ge $SENSOR2_WARN ]; then
	printf "WARNING: sensor2=$SENSOR2 "
	ERROR_SENSOR2=1

elif [ $SENSOR2 -lt $SENSOR2_WARN ]; then
	printf "OK: sensor2=$SENSOR2 "
	ERROR_SENSOR2=0

else
	# We should never get here
	printf "UNKNOWN: sensor2=$SENSOR2 "
	ERROR_SENSOR2=3
fi

### OUTPUT
printf ";|fanspeed=$(expr $FANSPEED)rpm;$FANSPEED_WARN;$FANSPEED_CRIT;0;9000 "
printf    "sensor1=${SENSOR1}C;$SENSOR1_WARN;$SENSOR1_CRIT;0;100 "
printf    "sensor2=${SENSOR2}C;$SENSOR2_WARN;$SENSOR2_CRIT;0;100\n"

#### ERROR handling
ERROR=$(printf $ERROR_FANSPEED $ERROR_SENSOR1 $ERROR_SENSOR2 | xargs -n1 | sort -n | tail -1)
exit $ERROR
