...
if   [ "$BATT_STATE" -lt "$BATT_CRIT" ]; then
	printf "%s" "CRITICAL: battery=$BATT_STATE "
	ERROR_BATTERY=2

elif [ "$BATT_STATE" -lt "$BATT_WARN" ]; then
	printf "%s" "WARNING: battery=$BATT_STATE "
	ERROR_BATTERY=1
else
	printf "%s" "OK: battery=$BATT_STATE "
	ERROR_BATTERY=0
fi

# Adapter (no WARNING?)
if   [ "$PWR_STATE" != "on-line" ]; then
	printf "%s" "CRITICAL: adapter=$PWR_STATE "
	ERROR_ADAPTER=2
else
	printf "%s" "OK: adapter=$PWR_STATE "
	ERROR_ADAPTER=0
fi

# Temperature
if   [ "$TEMP_STATE" -gt "$TEMP_CRIT" ]; then
	printf "%s" "CRITICAL: temperature=$TEMP_STATE "
	ERROR_TEMP=2

elif [ "$TEMP_STATE" -gt "$TEMP_WARN" ]; then
	printf "%s" "WARNING: temperature=$TEMP_STATE "
	ERROR_TEMP=1
else
	printf "%s" "OK: temperature=$TEMP_STATE "
	ERROR_TEMP=0
fi
